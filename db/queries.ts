...
import { challengeProgress, 
    ...
    userProgress, 
    lessons, 
    userSubscription } from "@/db/schema";
...
export const getNextLesson = cache(async (currentLessonId: number) => {
    ...
    // Get the current lesson to find its unit and order
    ...
    // Find the next lesson in the same unit (higher order)
    ...
    if (nextLessonInUnit) {
        return nextLessonInUnit;
    }
    
    // If no next lesson in current unit, find the next unit
    const nextUnit = await db.query.units.findFirst({
        where: and(
            eq(units.courseId, currentLesson.unit.courseId),
            eq(units.order, currentLesson.unit.order + 1)
        ),
        with: {
            lessons: {
                orderBy: (lessons, { asc }) => [asc(lessons.order)],
    ...
    // No next lesson found (end of course)
    return null;
});
